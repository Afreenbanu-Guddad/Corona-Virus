...
public class CoronaControllerTest {
	...
	public void getBystatenameTest() throws Exception {
		  Corona corona = new Corona("delhi", 2, 3, 2, 3, dateCreated);
	        ...
	        String url = "/covid/{satename}";
	        mockMvc.perform(get(url, "Delhi"))
	                .andExpect(status().isOk()).andDo(print()).andReturn();
	}
	
	// update
	@Test
	public void updateStateTest() throws JsonProcessingException, Exception {

	Corona corona = new Corona("Goa", 1, 7, 8, 9, dateCreated);
	when(service.getBystatename("Goa")).thenReturn(corona);
	when(service.updateState(corona, "Goa")).thenReturn(corona);
	Corona c=service.updateState(corona, corona.getStatename());
	...
	when(service.updateState(c, "Goa")).thenReturn(c);
	String url = "/covid/{statename}";
	mockMvc.perform(put(url,"Delhi").
